...
if __name__ == "__main__":
    X_train, y_train, X_test, y_test = load_mnist_direct()
    learning_rates = [0.005, 0.01, 0.05, 0.1, 0.25]
    os.makedirs("plots", exist_ok=True)
    
    plt.figure(figsize=(8, 5))
    for lr in learning_rates:
        print(f"Running MNIST Sweep: alpha = {lr}")
        history = train_framework(X_train, y_train, X_test, y_test, 784, 128, 15, lr, 128)
        plt.plot(range(1, 16), [a * 100 for a in history], marker='o', label=f"α = {lr}")
        
    plt.title("MNIST Convergence Sweep")
    plt.xlabel("Epoch")
    ...
    print("Saved plot to plots/mnist_sweep_metrics.png")
    plt.show()
